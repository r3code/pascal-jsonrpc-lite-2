...
var
  expected, actual: IJsonRpcParsedMessage;
  err: IJsonRpcError;
begin
  expected := TJsonRpcParsedMessage.Create(jotError,
    TJsonRpcErrorObject.Create('1', TJsonRpcError.Create(-32600,
    PRC_ERR_INVALID_REQUEST, 'blabla')));
  actual := TJsonRpcMessage.Parse(testMsg, err);
...
begin
  expected := TJsonRpcError.InvalidRequest(ERROR_NO_JSONRPC_FIELD);
  TJsonRpcMessage.Parse(testMsg, actual);

  CheckEquals(expected.AsJsonObject.AsJSon,
    actual.AsJsonObject.AsJSon());
end;
...
var
  ...
  err: IJsonRpcError;
begin
  expected := TJsonRpcParsedMessage.Create(jotNotification,
...
var
  ...
  err: IJsonRpcError;
begin
  expected := TJsonRpcParsedMessage.Create(jotRequest,
...
var
  ...
  err: IJsonRpcError;
begin
  expected := TJsonRpcParsedMessage.Create(jotSuccess,
